...
public class EventServicesImpl implements IEventServices{
    ...
    public Event addAffectEvenParticipant(Event event, int idParticipant) {
        ...
        try {
            participant = participantRepository.findById(idParticipant).orElseThrow(() -> new IllegalArgumentException("Participant with ID " + idParticipant + " not found."));
            ...
            throw new IllegalStateException("NullPointerException occurred while accessing participant's events", e);
        }
    ...
    public Event addAffectEvenParticipant(Event event) {
        ...
        for (Participant aParticipant : participants) {
            ...
            if (participant == null) {
                throw new IllegalArgumentException("Participant with ID " + aParticipant.getIdPart() + " not found.");
            }
